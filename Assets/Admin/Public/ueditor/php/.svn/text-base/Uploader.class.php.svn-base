...
class Uploader
{
    ...
    private $OSS_ACCESS_KEYID;
    private $OSS_ACCESS_KEYSECRET;
     ...
     * @return mixed
     */
    private function oss(){
        //读取config
        function C(){return '';}
        $cfg = include '../../../../../Admin/Admin/Conf/config.php';
        $this->OSS_ACCESS_KEYID = $cfg['OSS_ACCESS_KEYID'];
        $this->OSS_ACCESS_KEYSECRET = $cfg['OSS_ACCESS_KEYSECRET'];
        $this->OSS_ENDPOINT = $cfg['OSS_ENDPOINT'];
        $this->OSS_BUCKET = $cfg['OSS_BUCKET'];
        ...
        $img = base64_encode(file_get_contents($file['tmp_name']));
        $rd = rand(pow(10,(10-1)), pow(10,10)-1);
        $path = 'og/goodsinfo_'.time().'_'.$rd;
        $ret = $this->save_mem_img($img, $path, $type);
        //var_dump($ret);
        if (!$ret){
            $this->stateInfo = $this->getStateInfo("ERROR_UNKNOWN");
        } else {
            $this->stateInfo = $this->stateMap[0];
            $this->fullName = "http://{$this->OSS_ENDPOINT}/{$ret[0]}.{$ret[1]}";
        }
    }
     ...
     * 存储图片到阿里OSS
     */
    private function save_mem_img($img, $path, $attached_type){
        include '../../../../../Admin/Extend/OSS/OssClient.class.php';
        $img_base = $img;
        /*
        ...
        */
        if($attached_type!='') {
            ...
            try {
                $ossClient = new Extend\OSS\OssClient($this->OSS_ACCESS_KEYID, $this->OSS_ACCESS_KEYSECRET, $this->OSS_ENDPOINT, true);
            } catch (Extend\OSS\OssException $e) {
                //print $e->getMessage();
                return false;
            }
